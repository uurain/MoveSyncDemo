...
function GameMgrLogic:Init()
	...
	Event.AddListener(Msg.Id.AckSkillResult, handler(self.OnAckSkillResult, self))
...
function GameMgrLogic:OnAckPlayerLeaveList(msg)
	for i,v in ipairs(msg.object_list) do
		log("OnAckPlayerLeaveList:"..v)
		local actor = self.actorTable[v]
		if actor ~= nil then
			log("OnAckPlayerLeaveList2")
			actor:Dispose()
...
function GameMgrLogic:OnAckPublicPropertyList(msg)	
	local actor = self.actorTable[msg.id]
	if actor then
		actor:UpdateUnit(msg.player_info)
	end
end
...
function GameMgrLogic:OnAckSkillResult(msg)
	local atkActor = self.actorTable[msg.caster]
	for i,v in ipairs(msg.damages) do
		local behurtActor = self.actorTable[v.target]
		if behurtActor then
			behurtActor:OnBehurt(atkActor, v)
		end
	end
end
