...
namespace LuaFramework {
    public class GameManager : Manager {
        ...
        void Update()
        {
            ...
            {
                if ((Time.realtimeSinceStartup - preTouchTime) < 0.2f)
                {
                    Vector3 screenPos = Input.mousePosition;
                    Ray ray = Camera.main.ScreenPointToRay(screenPos);
                    if (Physics.Raycast(ray, out hitInfo, 200, LayerMask.GetMask("Terrain", "Actor")))
                    {
                        if (hitInfo.transform.gameObject.layer == LayerMask.NameToLayer("Terrain"))
                        {
                            Vector3 hitPoint = hitInfo.point;
                            LuaEventHandle.Call((int)EventEnum.SceneClick, hitPoint);
                        }
                        else
                        {
                        }
            ...
            {
                if (FairyGUI.Stage.inst != null && FairyGUI.Stage.inst.touchTarget != null)
                {

                }
                else
                {
                    preTouchTime = Time.realtimeSinceStartup;
                }
            }
        }
        ...
        void OnGUI()
        {
            ...
            GUI.Label(new Rect(100, Screen.height / 2, 1000, 100), LuaFramework.Main.updatePer);
        }
        ...
        void Init() {
            ...
            //OnResourceInited();
            //OnInitialize();
            //UpdateManager.CheckExtractResource(OnInitialize);
        ...
        IEnumerator OnExtractResource() {
            ...
            foreach (var file in files) {
                ...
                if (Application.platform == RuntimePlatform.Android) {
                    ...
                    if(www.error != "")
                    {
                        Debug.LogError(infile + ":" + www.error);
                    }
                    if (www.isDone) {
                        File.WriteAllBytes(outfile, www.bytes);
                    }
