...
namespace LuaFramework
{
    public class Util
    {
        ...
        //客户断与服务端时间查
        public static int SERVER_TIME_DIFF = 0;
        public static int GetServerTimeInt32()
        {
            return (int)(GetTimeSecond() + SERVER_TIME_DIFF);
        }

        public static string GetServerTime()
        {
            return DateTime.UtcNow.AddSeconds(SERVER_TIME_DIFF).ToString("yyyy-MM-dd HH:mm:ss");
        }
        ...
        public static GameObject Control(string name, GameObject parent)
        {
            if (parent == null)
            {
                //Debug.LogWarningFormat("Control name: \n{0}" + name, UGameClient.UGUtil.LuaTraceBack());
                return null;
            }
            GameObject obj = Control(name, parent, true);
            if (obj == null)
            {
                //Debug.LogWarningFormat("Control name: \n{0}" + name, UGameClient.UGUtil.LuaTraceBack());
            }
        ...
        private static GameObject Control(string name, GameObject parent, bool Inactive)
        {
        ...
        public static Color GetColorText(string colorStr)
        {
            string[] strs = colorStr.Split(':');
            if(strs.Length >= 3)
            {
                return new Color(int.Parse(strs[0]), int.Parse(strs[1]), int.Parse(strs[2]));
            }
            return Color.white;
        }
        ...
        public static string GetDirName(string Path)
        {
            ...
            StrDirPath = Path.Substring(0, Path.LastIndexOf('/'));
            return StrDirPath;
        }
